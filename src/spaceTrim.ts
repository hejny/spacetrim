...
export function spaceTrim(str: string /* TODO: , char: RegExp = /\s/*/): string {
    ...
    for (const line of lines) {
        const { startWhitespace, content } = line.match(/^(?<startWhitespace>\s*)(?<content>.*?)(\s*)$/)!.groups as any;
    ...
    str = lines
        .map((line) => line.substring(minStartWhitespaceLength!, minStartWhitespaceLength! + maxContentLength))
        .join('\n');
